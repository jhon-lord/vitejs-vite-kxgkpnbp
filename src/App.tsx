...
import {
  ...
  Pin, Target, ChevronRight, AlertTriangle, X, Save,
  ...
  Users, RefreshCw, Layers, CheckCircle, LogOut, Menu
} from "lucide-react";
      ...
      <div style={{background:T.card,borderRadius:"24px 24px 0 0",padding:24,width:"100%",maxWidth:480,maxHeight:"85vh",overflowY:"auto" as any,boxShadow:"0 -8px 40px rgba(0,0,0,0.3)",border:`1px solid ${T.border}`}}>
        ...
        {list.map((m:any)=>(
          <div key={m.id} style={{display:"flex",alignItems:"center",gap:10,marginBottom:10,padding:12,background:T.bg,borderRadius:12,border:`1px solid ${T.border}`}}>
            ...
            <div style={{flex:1}}><SI value={m.nome} onChange={(v:string)=>setList((p:any)=>p.map((x:any)=>x.id===m.id?{...x,nome:v}:x))} placeholder="Nome" T={T} style={{padding:"7px 10px",fontSize:12}}/></div>
            <div style={{display:"flex",gap:4}}>{MEMBRO_CORES.slice(0,5).map((c:string)=><div key={c} onClick={()=>setList((p:any)=>p.map((x:any)=>x.id===m.id?{...x,cor:c}:x))} style={{width:18,height:18,borderRadius:"50%",background:c,cursor:"pointer",border:m.cor===c?"3px solid #fff":"2px solid transparent",boxShadow:m.cor===c?"0 0 0 2px "+c:"none",transition:"all .15s"}}/>)}</div>
        ...
        {list.length<6&&<button onClick={()=>setList((p:any)=>[...p,{id:uid(),nome:"",cor:MEMBRO_CORES[p.length%MEMBRO_CORES.length]}])} style={{width:"100%",padding:"9px",background:"none",border:`1.5px dashed ${T.border}`,borderRadius:12,cursor:"pointer",color:T.sub,fontSize:12,fontWeight:600,display:"flex",alignItems:"center",justifyContent:"center",gap:6,marginBottom:14}}><Plus size={13}/>Adicionar membro</button>}
...
function Modal({tipo,form,setForm,onSave,onClose,editId,T,cartoes,getGastoCartao,membros,metas}: any) {
  ...
  const titles:any={entrada:"Nova Entrada",gasto_var:"Gasto Variável",gasto_fix:"Gasto Fixo",reserva:"Reserva",meta:"Nova Meta"};
            ...
            <div style={{display:"flex",flexDirection:"column" as any,gap:8}}>
              {metasList.map(([k,m]:any)=>{const p=Math.min((m.total/m.meta)*100,100);return(
                <div key={k} onClick={()=>f("metaId")(k)} style={{padding:"10px 14px",borderRadius:12,border:`2px solid ${form.metaId===k?"#6366f1":T.border}`,background:form.metaId===k?"#6366f108":T.bg,cursor:"pointer",transition:"all .15s"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><div style={{display:"flex",alignItems:"center",gap:7}}>{form.metaId===k&&<CheckCircle size={13} color="#6366f1"/>}<span style={{fontSize:13,fontWeight:700,color:form.metaId===k?"#6366f1":T.text}}>{m.nome}</span></div><span style={{fontSize:11,color:T.sub}}>{p.toFixed(0)}%</span></div>
                  ...
                  {form.metaId===k&&form.valor>0&&<div style={{marginTop:8,fontSize:11,color:"#10b981",fontWeight:600,display:"flex",alignItems:"center",gap:4}}><ArrowUpRight size={11}/>+{fmt(+form.valor)} → {fmt(m.total+(+form.valor||0))}</div>}
                </div>
              );})}
            </div>
          </FL>}
  ...
  return (
    <div>
      ...
      {limite>0&&<div style={{display:"flex",gap:8,marginTop:8}}>
        ...
        <div style={{flex:1,background:T.card,borderRadius:10,padding:"8px 12px",border:`1px solid ${T.border}`}}><div style={{fontSize:9,color:T.sub,marginBottom:2,textTransform:"uppercase" as any,letterSpacing:.5}}>Disponível</div><div style={{fontSize:13,fontWeight:800,color:"#10b981"}}>{fmt(Math.max(limite-gastoTotal,0))}</div></div>
      </div>}
    </div>
  );
}
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:2500,backdropFilter:"blur(4px)"}}>
      <div style={{background:T.card,borderRadius:"24px 24px 0 0",padding:24,width:"100%",maxWidth:480,maxHeight:"90vh",overflowY:"auto" as any,boxShadow:"0 -8px 40px rgba(0,0,0,0.3)",border:`1px solid ${T.border}`}}>
        ...
        <div style={{borderRadius:14,padding:"14px 18px",background:f.cor||CARD_GRADIENTS[0],marginBottom:18}}>
          ...
          <div style={{fontSize:10,color:"rgba(255,255,255,0.5)",marginTop:2}}>Limite: {fmt(+f.limite||0)}</div>
        </div>
...
const BOTTOM_MAIN=["dashboard","entradas","gastos","metas","relatorios"];

function Sidebar({tab,setTab,T,dark,toggleDark,membros,setShowMembros,onLogout,userEmail}: any) {
  ...
  return (
    <div style={{width:W,minHeight:"100vh",background:T.sidebar,borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column" as any,padding:"22px 0",flexShrink:0,position:"sticky" as any,top:0,height:"100vh",boxSizing:"border-box" as any,transition:"width .25s cubic-bezier(.4,0,.2,1)",overflow:"hidden"}}>
      ...
      {!col&&<div style={{margin:"0 10px 14px",padding:"10px 12px",borderRadius:12,background:T.navItemActive}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:membros.length>0?8:0}}>
          <div style={{width:28,height:28,borderRadius:"50%",background:"linear-gradient(135deg,#6366f1,#8b5cf6)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><User size={13} color="#fff"/></div>
          <div style={{overflow:"hidden",flex:1}}><div style={{fontSize:11,fontWeight:700,color:T.text,whiteSpace:"nowrap" as any,overflow:"hidden",textOverflow:"ellipsis"}}>{userEmail}</div><div style={{fontSize:9,color:T.sub}}>Minha conta</div></div>
        </div>
        {membros.length>0&&<div style={{cursor:"pointer",paddingTop:8,borderTop:`1px solid ${T.border}`}} onClick={()=>setShowMembros(true)}>
          <div style={{fontSize:9,color:T.sub,marginBottom:4,textTransform:"uppercase" as any,letterSpacing:.5,fontWeight:700}}>Membros</div>
          <div style={{display:"flex",alignItems:"center",gap:2}}>{membros.slice(0,4).map((m:any,i:number)=><Avatar key={m.id} membro={m} size={20} style={{marginLeft:i===0?0:-4,border:`2px solid ${T.sidebar}`}}/>)}<span style={{fontSize:10,color:T.sub,marginLeft:6}}>{membros.length}</span></div>
        </div>}
      </div>}
      ...
      <div style={{padding:"12px 8px 0",borderTop:`1px solid ${T.border}`}}>
        <button onClick={toggleDark} style={{...C(false),marginBottom:4}}>
          {dark?<Sun size={16}/>:<Moon size={16}/>}{!col&&(dark?"Modo Claro":"Modo Escuro")}
        </button>
        <button onClick={onLogout} style={{...C(false),marginBottom:0,color:"#ef4444"}}>
          <LogOut size={16}/>{!col&&"Sair"}
        </button>
      </div>
    </div>
  );
...
function BottomNav({tab,setTab,T}: any) {
  ...
  const mainItems=NAVITEMS.filter(([k]:any)=>BOTTOM_MAIN.includes(k));
  const moreItems=NAVITEMS.filter(([k]:any)=>!BOTTOM_MAIN.includes(k));
  const activeInMore=moreItems.some(([k]:any)=>k===tab);
  return (<>
    <div style={{position:"fixed",bottom:0,left:0,right:0,background:T.sidebar,borderTop:`1px solid ${T.border}`,display:"flex",zIndex:200,paddingBottom:"env(safe-area-inset-bottom,0px)"}}>
      {mainItems.map(([k,ic,l]:any)=>(
        <button key={k} onClick={()=>{setTab(k);setShowMore(false);}} style={{flex:1,display:"flex",flexDirection:"column" as any,alignItems:"center",gap:2,padding:"9px 0 7px",border:"none",cursor:"pointer",background:"transparent",color:tab===k?T.accent:T.navText,position:"relative" as any}}>
      ...
      <button onClick={()=>setShowMore(!showMore)} style={{flex:1,display:"flex",flexDirection:"column" as any,alignItems:"center",gap:2,padding:"9px 0 7px",border:"none",cursor:"pointer",background:"transparent",color:activeInMore||showMore?T.accent:T.navText,position:"relative" as any}}>
        <Menu size={18} style={{opacity:activeInMore||showMore?1:.6}}/>
        <span style={{fontSize:9,fontWeight:activeInMore||showMore?700:500}}>Mais</span>
        {activeInMore&&<div style={{position:"absolute",top:0,left:"50%",transform:"translateX(-50%)",width:18,height:2,borderRadius:99,background:T.accent}}/>}
      </button>
    </div>
    {showMore&&<>
      <div onClick={()=>setShowMore(false)} style={{position:"fixed",inset:0,zIndex:190}}/>
      <div style={{position:"fixed",bottom:62,right:8,background:T.card,borderRadius:16,border:`1px solid ${T.border}`,boxShadow:"0 -4px 24px rgba(0,0,0,0.3)",zIndex:210,minWidth:160,overflow:"hidden"}}>
        {moreItems.map(([k,ic,l]:any)=>(
          <button key={k} onClick={()=>{setTab(k);setShowMore(false);}} style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:"12px 16px",border:"none",cursor:"pointer",background:tab===k?T.navItemActive:"transparent",color:tab===k?T.navTextActive:T.navText,fontSize:13,fontWeight:tab===k?700:500,borderBottom:`1px solid ${T.border}`}}>
            <span style={{opacity:tab===k?1:.7}}>{ic}</span>{l}
          </button>
        ))}
      </div>
    </>}
...
function MobileHeader({T,dark,toggleDark,mes,setMes,tab,membros,setShowMembros,onLogout}: any) {
    ...
    <div style={{background:T.sidebar,borderBottom:`1px solid ${T.border}`,padding:"11px 14px",position:"sticky" as any,top:0,zIndex:100}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:9}}>
        ...
        <div style={{display:"flex",gap:5,alignItems:"center"}}>
          ...
          <button onClick={onLogout} style={{background:"#fee2e2",border:"none",cursor:"pointer",borderRadius:7,width:28,height:28,display:"flex",alignItems:"center",justifyContent:"center",color:"#ef4444"}}><LogOut size={13}/></button>
        </div>
      </div>
...
export default function App() {
  ...
  const loadData=async()=>{
    ...
    const {data}=await supabase.from("user_data").select("data").eq("user_id",session.user.id).single();
    if(data?.data){
      const d=data.data;
      ...
      if(d.membros)setMembros(d.membros);
      if(d.entradasFixas)setEntradasFixas(d.entradasFixas);
      if(d.dark!==undefined)setDark(d.dark);
    }
  };

  const persist=useCallback(async(data:any,m:any,c:any,o:any,mb:any,ef:any,dk?:boolean)=>{
    if(!session?.user)return;
    const payload={allData:data,metas:m,cartoes:c,orcamentos:o,membros:mb,entradasFixas:ef,dark:dk??dark};
    await supabase.from("user_data").upsert({user_id:session.user.id,data:payload},{onConflict:"user_id"});
  },[session,dark]);

  const toggleDark=()=>setDark(d=>{const nd=!d;persist(allData,metas,cartoes,orcamentos,membros,entradasFixas,nd);return nd;});
  ...
  const del=(tipo:string,id:string,item?:any)=>{
    ...
    if(tipo==="reserva"){
      const reserva=D.reservas.find((r:any)=>r.id===id);
      if(reserva?.metaId&&reserva.valor){setMetas((prev:any)=>{const n={...prev,[reserva.metaId]:{...prev[reserva.metaId],total:Math.max(0,(prev[reserva.metaId]?.total||0)-(+reserva.valor))}};persist(allData,n,cartoes,orcamentos,membros,entradasFixas);return n;});}
      setD((d:any)=>({...d,reservas:d.reservas.filter((e:any)=>e.id!==id)}));
    }
  };
  ...
  if(authLoading) return (
    <div style={{minHeight:"100vh",background:"#0d1117",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column" as any,gap:16}}>
      <div style={{width:48,height:48,borderRadius:14,background:"linear-gradient(135deg,#6366f1,#8b5cf6)",display:"flex",alignItems:"center",justifyContent:"center"}}><Zap size={24} color="#fff"/></div>
      <div style={{fontSize:14,color:"#8892a4"}}>Carregando...</div>
    </div>
  );
  ...
  return (
    <div style={{display:"flex",fontFamily:"'Inter',system-ui,sans-serif",background:T.bg,minHeight:"100vh",transition:"background .3s"}}>
      {!isMob&&<Sidebar tab={tab} setTab={setTab} T={T} dark={dark} toggleDark={toggleDark} membros={membros} setShowMembros={setShowMembros} onLogout={handleLogout} userEmail={session.user.email}/>}
      <div style={{flex:1,display:"flex",flexDirection:"column" as any,minHeight:"100vh",overflow:"auto"}}>
        {isMob&&<MobileHeader T={T} dark={dark} toggleDark={toggleDark} mes={mes} setMes={setMes} tab={tab} membros={membros} setShowMembros={setShowMembros} onLogout={handleLogout}/>}
        ...
        <div style={{padding:P,flex:1,paddingBottom:isMob?"88px":P}}>
          ...
          {tab==="entradas"&&(
            <GC T={T}><SecHdr icon={<TrendingUp size={14} color="#10b981"/>} label={`Entradas — ${MESES[mes]}`} btnLabel="Adicionar" btnAction={()=>openAdd("entrada")} color="#10b981" extra={entradasFixas.length>0&&<div style={{display:"flex",alignItems:"center",gap:5,padding:"5px 10px",background:"#6366f118",borderRadius:8,fontSize:11,color:"#6366f1",fontWeight:600}}><RefreshCw size={11}/>{entradasFixas.length} fixo(s)</div>}/>
              <div style={{overflowX:"auto" as any}}>
                <table style={{width:"100%",borderCollapse:"collapse" as any,minWidth:300}}>
          ...
          {tab==="gastos"&&(
            <GC T={T}><SecHdr icon={<ShoppingCart size={14} color="#ef4444"/>} label={`Gastos Variáveis — ${MESES[mes]}`} btnLabel="Adicionar" btnAction={()=>openAdd("gasto_var")} color="#ef4444" extra={<GhostBtn onClick={()=>setShowOrcModal(true)} T={T} color={T.accent}><Bell size={12}/>Orçamentos</GhostBtn>}/>
              <div style={{overflowX:"auto" as any}}>
                <table style={{width:"100%",borderCollapse:"collapse" as any,minWidth:480}}>
                  ...
                  <tbody>
                    ...
                    {D.gastos_var.map((e:any)=><tr key={e.id} style={{background:e.parcelado?"#ef444406":""}}>
                      ...
                      <td style={td}>{e.parcelado?<span style={{fontSize:9,background:"#ef4444",color:"#fff",borderRadius:4,padding:"1px 5px",fontWeight:700}}>{e.parcelaNum}/{e.parcelaTotal}</span>:e.data||"—"}</td>
                      {actBtns("gasto_var",e)}
                    </tr>)}
          ...
          {tab==="reservas"&&(
            <GC T={T}><SecHdr icon={<PiggyBank size={14} color="#6366f1"/>} label={`Reservas — ${MESES[mes]}`} btnLabel="Adicionar" btnAction={()=>openAdd("reserva")} color="#6366f1"/>
              <div style={{overflowX:"auto" as any}}>
                <table style={{width:"100%",borderCollapse:"collapse" as any,minWidth:360}}>
                  <thead><tr><th style={th}>Quem</th><th style={th}>Nome</th><th style={th}>Tipo</th><th style={th}>Meta</th><th style={th}>Valor</th><th style={th}/></tr></thead>
                  <tbody>
                    ...
                    {D.reservas.map((e:any)=><tr key={e.id}>
                      ...
                      <td style={td}>{e.metaId&&metas[e.metaId]?<span style={{fontSize:11,color:"#8b5cf6",fontWeight:600}}>{metas[e.metaId].nome}</span>:"—"}</td>
          ...
          {tab==="relatorios"&&(<>
            <GC T={T} style={{marginBottom:14}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap" as any,gap:8}}>
                <div><h3 style={{margin:0,fontSize:14,fontWeight:800,color:T.text}}>Resumo Anual {ANO_ATUAL}</h3><div style={{fontSize:10,color:T.sub}}>Visão completa do ano</div></div>
              ...
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={annualData.map(d=>({...d,mes:d.mes.slice(0,3)}))}>
                  <XAxis dataKey="mes" axisLine={false} tickLine={false} tick={{fontSize:9,fill:T.sub}}/><YAxis hide/>
                  <Tooltip contentStyle={{background:T.card,border:`1px solid ${T.border}`,borderRadius:10,fontSize:11,color:T.text}} formatter={(v:any)=>fmt(v)}/>
                  <Line type="monotone" dataKey="entradas" stroke="#6366f1" strokeWidth={2} dot={{r:2}} name="Entradas"/>
                  <Line type="monotone" dataKey="saidas" stroke="#ef4444" strokeWidth={2} dot={{r:2}} name="Saídas"/>
                  <Line type="monotone" dataKey="reservas" stroke="#10b981" strokeWidth={2} dot={{r:2}} name="Reservas"/>
                </LineChart>
              </ResponsiveContainer>
            </GC>
            <GC T={T}>
              <h3 style={{margin:"0 0 14px",fontSize:13,fontWeight:800,color:T.text}}>Detalhamento por Mês</h3>
              <div style={{overflowX:"auto" as any}}>
                <table style={{width:"100%",borderCollapse:"collapse" as any,minWidth:400}}>
                  ...
                  <tbody>
                    ...
                    <tr style={{background:T.rowAlt,borderTop:`2px solid ${T.border}`}}>
                      <td style={{...td,fontWeight:800}}>Total Ano</td>
      ...
      {showOrcModal&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:2000,backdropFilter:"blur(4px)"}}>
        <div style={{background:T.card,borderRadius:"24px 24px 0 0",padding:24,width:"100%",maxWidth:480,maxHeight:"80vh",overflowY:"auto" as any,boxShadow:"0 -8px 40px rgba(0,0,0,0.3)",border:`1px solid ${T.border}`}}>
          ...
          <p style={{fontSize:12,color:T.sub,marginBottom:12}}>Alerta quando atingir 80%.</p>
...
// Bottom nav com 5 itens principais + "Mais"
const BOTTOM_MAIN = ["dashboard","entradas","gastos","metas","relatorios"];
...
function Sidebar({tab,setTab,T,dark,toggleDark,membros,setShowMembros,onLogout,userEmail}: any) {
  ...
  return (
    <div style={{width:W,minHeight:"100vh",background:T.sidebar,borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column" as any,padding:"22px 0",flexShrink:0,position:"sticky" as any,top:0,height:"100vh",boxSizing:"border-box" as any,transition:"width .25s cubic-bezier(.4,0,.2,1)",overflow:"hidden"}}>
      ...
      {!col&&<div style={{margin:"0 10px 14px",padding:"10px 12px",borderRadius:12,background:T.navItemActive}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <div style={{width:30,height:30,borderRadius:"50%",background:"linear-gradient(135deg,#6366f1,#8b5cf6)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><User size={14} color="#fff"/></div>
          <div style={{overflow:"hidden",flex:1}}>
            <div style={{fontSize:11,fontWeight:700,color:T.text,whiteSpace:"nowrap" as any,overflow:"hidden",textOverflow:"ellipsis"}}>{userEmail}</div>
            <div style={{fontSize:9,color:T.sub}}>Minha conta</div>
          </div>
        </div>
        {membros.length>0&&<div style={{marginTop:8,paddingTop:8,borderTop:`1px solid ${T.border}`,cursor:"pointer"}} onClick={()=>setShowMembros(true)}>
          <div style={{fontSize:9,color:T.sub,marginBottom:4,textTransform:"uppercase" as any,letterSpacing:.5,fontWeight:700}}>Membros</div>
          <div style={{display:"flex",alignItems:"center",gap:2}}>
            {membros.slice(0,4).map((m:any,i:number)=><Avatar key={m.id} membro={m} size={20} style={{marginLeft:i===0?0:-4,border:`2px solid ${T.sidebar}`}}/>)}
            <span style={{fontSize:10,color:T.sub,marginLeft:6}}>{membros.length}</span>
          </div>
        </div>}
      </div>}
...
// Mobile bottom nav com drawer "Mais"
function BottomNav({tab,setTab,T}: any) {
  const [showMore,setShowMore]=useState(false);
  const mainItems=NAVITEMS.filter(([k]:any)=>BOTTOM_MAIN.includes(k));
  const moreItems=NAVITEMS.filter(([k]:any)=>!BOTTOM_MAIN.includes(k));
  ...
  return (<>
    ...
    {showMore&&<>
      <div onClick={()=>setShowMore(false)} style={{position:"fixed",inset:0,zIndex:190}}/>
      <div style={{position:"fixed",bottom:60,right:8,background:T.card,borderRadius:16,border:`1px solid ${T.border}`,boxShadow:"0 -4px 24px rgba(0,0,0,0.3)",zIndex:210,minWidth:160,overflow:"hidden"}}>
        {moreItems.map(([k,ic,l]:any)=>(
          <button key={k} onClick={()=>{setTab(k);setShowMore(false);}} style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:"12px 16px",border:"none",cursor:"pointer",background:tab===k?T.navItemActive:"transparent",color:tab===k?T.navTextActive:T.navText,fontSize:13,fontWeight:tab===k?700:500,borderBottom:`1px solid ${T.border}`}}>
            <span style={{opacity:tab===k?1:.7}}>{ic}</span>{l}
          </button>
...
export default function App() {
  ...
  const del=(tipo:string,id:string,item?:any)=>{
    if(tipo==="entrada_fixa"){const next=entradasFixas.filter((e:any)=>e.id!==id);setEntradasFixas(next);persist(allData,metas,cartoes,orcamentos,membros,next);return;}
    if(tipo==="entrada")setD((d:any)=>({...d,entradas:d.entradas.filter((e:any)=>e.id!==id)}));
    if(tipo==="gasto_var"){if(item?.parcelaId){setAllData((prev:any)=>{const next:any={};MESES.forEach((_,i)=>{next[i]={...prev[i],gastos_var:prev[i].gastos_var.filter((e:any)=>e.parcelaId!==item.parcelaId)};});persist(next,metas,cartoes,orcamentos,membros,entradasFixas);return next;});return;}setD((d:any)=>({...d,gastos_var:d.gastos_var.filter((e:any)=>e.id!==id)}));}
    if(tipo==="gasto_fix")setD((d:any)=>({...d,gastos_fix:d.gastos_fix.filter((e:any)=>e.id!==id)}));
